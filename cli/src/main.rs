...
use etf::runtime_types::{
    ...
    node_template_runtime::{RuntimeCall, RuntimeCall::Balances},
};

use murmur_core::{
    murmur::MurmurStore,
    types::{BlockNumber, Identity, IdentityBuilder, Leaf},
};
use rand_core::OsRng;
use ark_serialize::CanonicalSerialize;
use w3f_bls::{KeypairVT, DoublePublicKey, DoublePublicKeyScheme, SerializableToBytes, TinyBLS377};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let keypair = KeypairVT::<TinyBLS377>::generate(&mut OsRng);
    let double_public: DoublePublicKey<TinyBLS377> = DoublePublicKey(
        keypair.into_public_key_in_signature_group().0,
        keypair.public.0,
    );
    let mut round_pubkey_bytes = Vec::new();
    double_public.serialize_compressed(&mut round_pubkey_bytes).unwrap();
    ...
    match &cli.commands {
        Commands::New(args) => {
            // if args.x {
                let (etf, block_number, pk_bytes) = idn_connect().await?;
                current_block_number = block_number;
                round_pubkey_bytes = pk_bytes;
            // }
            ...
            // println!("seriously wtf: {:?}", mmr_store.metadata.clone());
            // 3. add to storage
...
async fn idn_connect(
    ...
    let etf = OnlineClient::<SubstrateConfig>::from_rpc_client(rpc_client.clone()).await?;
    ...
    Ok((etf, current_block_number, round_pubkey_bytes))
}
...
pub async fn prepare_execute(
    ...
    let (proof, commitment, ciphertext, pos) = store
        ...
        .unwrap();
    let test = proof
        .verify(store.root, vec![(pos.clone(), Leaf(ciphertext.clone()))])
        .unwrap();
    println!("valid ? {:?}", test);
    // println!("MMR ROOT:{:?}", store.root.clone());

    let size: u64 = proof.mmr_size() as u64;
...
//     #[test]
//     pub fn it_can_create_mmr_store_and_verifiable_execution_params() {
//         let name = "name".to_string();
//         let seed = "seed".to_string();
//         let ephem_msk = [1;32];
//         let schedule = vec![1,2,3,4,5,6,7,8,9];
...
//         assert!(write_mmr_store(mmr_store.clone(), "mmr_store").is_ok());

//         // then try to load the mmr_store
//     }
// }
